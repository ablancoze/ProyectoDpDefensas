 ...
 */
public class ManoIzquierda implements MoveBehavior {
	...
	public LinkedList<Dir> move(int origen,int destino) {
		...
		if(ruta.isEmpty()){
			...
			if(Mapa.getMapa().comprobarMovimiento(origen, origen+1)){//Vemos si podemos girar a la izquierda
				origen=origen+1;
	...
	/**
	 * Metodo privado que continua la generacion de la ruta de mano izquierda
